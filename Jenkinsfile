...
pipeline {
    ...
    environment {
        COMPOSE_PROJECT_NAME = "inventorymanagement${BRANCH_NAME}"
    }
    parameters {
    ...
    stages {
        ...
        stage('Deploy to test') {
            ...
            steps {
                build job: 'OpenLMIS-inventorymanagement-deploy-to-test', wait: false
            }
            post {
                failure {
                    script {
        ...
        stage('ERD generation') {
            ...
            steps {
                dir('erd') {
                    ...
                    archiveArtifacts artifacts: 'erd-inventorymanagement.zip'
                }
            }
            post {
                failure {
        ...
        stage('Push image') {
            ...
            steps {
                sh "docker pull openlmis/inventorymanagement:${STAGING_VERSION}"
                sh "docker tag openlmis/inventorymanagement:${STAGING_VERSION} openlmis/inventorymanagement:${VERSION}"
                sh "docker push openlmis/inventorymanagement:${VERSION}"
            }
            post {
                success {
                    script {
