...
docker pull openlmis/inventorymanagement
...
sudo mkdir -p /var/www/html/erd-inventorymanagement
sudo chown -R $USER:$USER /var/www/html/erd-inventorymanagement
...
&& wget https://raw.githubusercontent.com/OpenLMIS/openlmis-inventorymanagement/master/docker-compose.erd-generation.yml -O docker-compose.yml \
&& (/usr/local/bin/docker-compose up &) \
&& sleep 90 \
&& sudo rm /var/www/html/erd-inventorymanagement/* -rf \
&& mkdir output \
&& chmod 777 output \
&& (docker run --rm --network openlmisServicePointerdgeneration_default -v $WORKSPACE/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s inventorymanagement -u postgres -p $DBPASSWORD -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
&& sleep 30 \
&& /usr/local/bin/docker-compose down --volumes \
&& sudo chown -R $USER:$USER output \
&& mv output/* /var/www/html/erd-inventorymanagement \
&& rm erd-inventorymanagement.zip -f \
&& pushd /var/www/html/erd-inventorymanagement \
&& zip -r $WORKSPACE/erd-inventorymanagement.zip . \
&& popd \
&& rmdir output \
&& rm .env \
